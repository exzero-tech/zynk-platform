...
export default function BottomNavbar() {
  ...
  const buttonClass = (tab: string) =>
    `flex flex-col items-center px-4 py-2 rounded-full transition-all duration-200 cursor-pointer ${
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-foreground rounded-full mx-4 mb-4 p-2 flex justify-around items-center z-[1000]">
      ...
      <button className={buttonClass('explore')} onClick={() => handleNavigation('/explore')}>
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
